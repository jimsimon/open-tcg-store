...
export function render(ctx: RouterContext) {
  ...
  return `
    <ogs-settings-users-page userRole="${escapeHtml(userRole)}" ${isAnonymous ? 'isAnonymous' : ''} userName="${escapeHtml(userName)}" activePage="settings/users" showUserMenu></ogs-settings-users-page>
  `;
}
