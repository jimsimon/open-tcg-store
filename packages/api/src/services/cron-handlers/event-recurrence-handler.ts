...
import { getRecurrenceTemplates, generateRecurrenceInstances } from '../event-service.ts';
...
export async function eventRecurrenceHandler(config: Record<string, unknown>): Promise<JobResult> {
  const windowWeeks = typeof config.windowWeeks === 'number' ? config.windowWeeks : 8;
